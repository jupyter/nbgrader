...
def test_ignore(temp_cwd):
    ...
    files.append("foo")
    assert utils.ignore(exclude=["foo*"])(dir, files) == ["foo.txt", "foo"]
    assert utils.ignore(include=["*.txt"])(dir, files) == ["truc.png"]
    assert utils.ignore(exclude=["foo.*"], include=["*.txt"])(dir, files) == ['foo.txt', 'truc.png']
    assert utils.ignore(max_file_size=2)(dir, files) == ["long.txt"]
