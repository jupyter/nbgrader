...
def _make_nbserver(course_id, port, tempdir, jupyter_config_dir,
                   jupyter_data_dir, exchange, cache, startup_fn=None):
    env = os.environ.copy()
    ...
    with open('nbgrader_config.py', 'w') as fh:
        fh.write(dedent(
            ...
            c.CourseDirectory.db_assignments = [dict(name="Problem Set 1"),
                                                dict(name="ps.01")]
...
def _make_browser(tempdir):
    for filename in glob.glob(os.path.join(os.path.dirname(__file__), "files",
                                           "*.ipynb")):
        shutil.copy(
                filename, os.path.join(tempdir, os.path.basename(filename)))
