...
from ...nbgraderformat.v4 import (
    MetadataValidatorV4, read_v4, reads_v4, write_v4, writes_v4)
from .. import (
    create_code_cell,
    create_grade_cell,
...
def test_set_false():
    ...
    MetadataValidatorV4().upgrade_cell_metadata(cell)
...
def test_extra_keys():
    ...
    cell = create_grade_cell("", "code", "foo", "", 3)
    cell.metadata.nbgrader["foo"] = "bar"
    MetadataValidatorV4().upgrade_cell_metadata(cell)
    assert "foo" not in cell.metadata.nbgrader
...
def test_schema_version():
    ...
    MetadataValidatorV4().upgrade_cell_metadata(cell)
    assert cell.metadata.nbgrader["schema_version"] == 4
...
def test_reads():
    ...
    reads_v4(contents, current_nbformat)


def test_write():
    currdir = os.path.split(__file__)[0]
    path = os.path.join(currdir, "..", "apps", "files", "test.ipynb")
    nb = read_v4(path, current_nbformat)
...
def test_upgrade_notebook_metadata():
    ...
    nb = MetadataValidatorV4().upgrade_notebook_metadata(nb)
...
def test_upgrade_cell_metadata():
    ...
    cell = create_grade_cell("", "code", "foo", 5, 10)
    MetadataValidatorV4().upgrade_cell_metadata(cell)


def test_regular_cells():
    validator = MetadataValidatorV4()
    ...
    cell = create_regular_cell("", "code", schema_version=4)
    validator.validate_cell(cell)

    nb = new_notebook()
    cell1 = create_code_cell()
    cell2 = create_regular_cell("", "code", schema_version=4)
...
def test_invalid_metadata():
    ...
    # grade_id is empty
    cell = create_grade_cell("", "code", "", 5, 4)
    ...
    cell = create_grade_cell("", "markdown", "foo", 5, 4)
    with pytest.raises(ValidationError):
        validator.validate_cell(cell)

    # markdown solution cell not marked as a grade cell
    cell = create_solution_cell("", "markdown", "foo", 4)
    with pytest.raises(ValidationError):
        validator.validate_cell(cell)

    # task cell shouldn't be a code cell
    cell = create_task_cell("", "markdown", "foo", 5, 4)
    validator.validate_cell(cell)

    # task cell shouldn't be able to be a code cell
    cell = create_task_cell("", "code", "foo", 5, 4)
    with pytest.raises(ValidationError):
        validator.validate_cell(cell)
...
def test_duplicate_cells():
    validator = MetadataValidatorV4()
    nb = new_notebook()
    cell1 = create_grade_cell("", "code", "foo", 5, 4)
    cell2 = create_grade_cell("", "code", "foo", 5, 4)
    nb.cells = [cell1, cell2]
    with pytest.raises(ValidationError):
        validator.validate_nb(nb)
...
def test_celltype_changed(caplog):
    cell = create_solution_cell("", "code", "foo", 4)
    cell.metadata.nbgrader["cell_type"] = "code"
    MetadataValidatorV4().validate_cell(cell)
